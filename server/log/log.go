...
type ILog interface {
	addSTDHook(int)
	addMDBHook(*mdbHook.MongoConfig, int)
...
// addMDBHook : initialize terminal hook
func (l *Log) addMDBHook(cfg *mdbHook.MongoConfig, level int) {
	l.dbHook = new(mdbHook.Hook)
	l.dbHook.Init(cfg, level)
	l.logger.AddHook(l.dbHook)
}
