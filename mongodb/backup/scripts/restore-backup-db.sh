...
docker exec -t itv-mongo mongorestore $MONGO_PATH/$DEFAULT_DB --host ${DEFAULT_HOST} --port ${DEFAULT_PORT} --drop --db ${DEFAULT_DB} --username ${DEFAULT_USER} --password ${DEFAULT_PASS} --authenticationDatabase admin